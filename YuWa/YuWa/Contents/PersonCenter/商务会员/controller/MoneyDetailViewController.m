...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==0) {
        ...
        titleLabel.text = @"结算周期";
        
        detailLabel.text = [NSString stringWithFormat:@"%@至%@",[JWTools getTimeTwo:self.model.ctime],[JWTools getTimeTwo:self.model.pay_to_shop_time]];
        detailLabel.numberOfLines = 0;
        
    }else if (indexPath.row==4){
        titleLabel.text = @"付款状态";
        detailLabel.text = self.model.order_type;

//        titleLabel.text=@"付款总价";//待修改
