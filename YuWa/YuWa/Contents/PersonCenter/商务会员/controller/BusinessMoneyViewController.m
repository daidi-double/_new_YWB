...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section==0) {
        ...
        if ([self.model.my_shop_nums  isKindOfClass:[NSNull class]]) {
            totailLabel.text = @"0";
        }
        if ([self.model.total_business  isKindOfClass:[NSNull class]]) {
            totailLabel.text = @"0.00";
        }
        if ([self.model.today_business  isKindOfClass:[NSNull class]]) {
            todayLabel.text = @"0.00";
        }
        if ([self.model.settlement_business  isKindOfClass:[NSNull class]]) {
            waitLabel.text = @"0.00";
        }
        return cell;
