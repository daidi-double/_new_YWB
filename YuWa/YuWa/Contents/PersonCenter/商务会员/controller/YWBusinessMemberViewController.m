...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        ...
        if ([topLabel.text isEqualToString:@"(null)"]) {
            topLabel.text = @"0";
        }
        subLabel.text=@"门店数量";
        timeLabel.text=[JWTools currentTime];
    ...
    if ([waitLabel.text isEqualToString:@"(null)"]) {
        waitLabel.text = @"0.00";
    }
    if ([totailLabel.text isEqualToString:@"(null)"]) {
        totailLabel.text = @"0.00";
    }
    
    if ([todayLabel.text isEqualToString:@"(null)"] || [todayLabel.text isEqualToString:@""]) {
...
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (section==0) {
        ...
        if ([label2.text isEqualToString:@""]||[label2.text isEqualToString:@"(null)"]) {
            label2.text = @"0.00";
        }
        ...
        label5.text=@"积分";
        [label5 bringSubviewToFront:self.view];
        UILabel*label6=[view viewWithTag:6];
        label6.text=self.self.scoreModel.total_score;
...
-(void)getDatas{
    ...
    [manager postDatasWithUrl:urlStr withParams:params compliation:^(id data, NSError *error) {
        ...
        if ([errorCode isEqualToString:@"0"]) {
            ...
            self.businessModel=[BusinessMoneyModel yy_modelWithDictionary:data[@"data"][@"business"]];
