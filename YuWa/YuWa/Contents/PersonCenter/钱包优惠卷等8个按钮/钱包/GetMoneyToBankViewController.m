...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    GetMoneyToBankModel * model = self.dataAry[0];
    if (indexPath.row == 1) {
        NSString * card = [model.card substringFromIndex:15];
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ 尾号%@",model.card_type,card];
    ...
    cell.detailTextLabel.text = model.money;
      ...
      cell.detailTextLabel.text = model.fee;
        ...
        cell.detailTextLabel.text = model.forecastTime;
    }else{
...
- (void)requestData{
    NSString * urlStr = [NSString stringWithFormat:@"%@%@",HTTP_ADDRESS,HTTP_GETMYMONEYTOBANK];
    ...
    [manager postDatasNoHudWithUrl:urlStr withParams:pragrams compliation:^(id data, NSError *error) {
        ...
        if (number == 0) {
            ...
            NSDictionary * dict = data[@"data"];
                GetMoneyToBankModel * model = [GetMoneyToBankModel modelWithdic:dict];
