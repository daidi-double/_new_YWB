...
- (void)requesData{
    ...
    NSDictionary * pragram = @{@"user_id":@([UserSession instance].uid),@"device_id":[JWTools getUUID],@"token":[UserSession instance].token,@"phone":self.phoneNumber,@"user_type":@(1)};
...
- (void)requestBankCode{
    NSString * urlStr = [NSString stringWithFormat:@"%@%@",HTTP_ADDRESS,HTTP_PRESON_ADDBANK];

    NSDictionary * pragram = @{@"user_id":@([UserSession instance].uid),@"device_id":[JWTools getUUID],@"token":[UserSession instance].token,@"user_type":@(1),@"card":_bankNumber,@"card_type":_bankCategory,@"phone":_phoneNumber,@"moible_code":securityCodeTF.text,@"user_name":_userName};
    HttpManager * manager = [[HttpManager alloc]init];
    [manager postDatasWithUrl:urlStr withParams:pragram compliation:^(id data, NSError *error) {
        ...
        if (errorCode == 0) {
            AddBankSuccessViewController * successVC = [[AddBankSuccessViewController alloc]init];
            ...
            [JRToast showWithText:data[@"errorCode"]];
        }
