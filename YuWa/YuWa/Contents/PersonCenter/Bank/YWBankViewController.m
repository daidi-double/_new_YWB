...
#pragma mark - Http
- (void)requestData{
    NSString * urlStr = [NSString stringWithFormat:@"%@%@",HTTP_ADDRESS,HTTP_PRESON_BANKLIST];
    NSDictionary * pragram = @{@"user_id":@([UserSession instance].uid),@"device_id":[JWTools getUUID],@"token":[UserSession instance].token,@"user_type":@(1)};
...
- (void)checkUserWhetherHavePassword{
    ...
    NSDictionary * pragrams = @{@"device_id":[JWTools getUUID],@"user_id":@([UserSession instance].uid),@"token":[UserSession instance].token,@"user_type":@(1)};
