//
//  AddBankSuccessViewController.m
//  YuWaShop
//
...
@interface AddBankSuccessViewController ()<UIGestureRecognizerDelegate>
@property (weak, nonatomic) IBOutlet UIView *BGView;
@property (weak, nonatomic) IBOutlet UILabel *todayMoneyMaxLabel;//今日最大额度
@property (weak, nonatomic) IBOutlet UILabel *everyMoneyMaxLabel;//单笔最大额度
@property (nonatomic,strong)NSString * phoneCode;
@end

@implementation AddBankSuccessViewController
...
-(void)relieveBankCard{
    ...
    NSDictionary * pragram = @{@"user_id":@([UserSession instance].uid),@"device_id":[JWTools getUUID],@"token":[UserSession instance].token,@"user_type":@(1),@"id":self.bank_id,@"moible_code":self.phoneCode};
...
- (void)requestPhoneCode{
    NSString * urlStr = [NSString stringWithFormat:@"%@%@",HTTP_ADDRESS,HTTP_PRESON_BANKCODE];
    NSDictionary * pragram = @{@"user_id":@([UserSession instance].uid),@"device_id":[JWTools getUUID],@"token":[UserSession instance].token,@"phone":self.phoneNumber};
    HttpManager * manager = [[HttpManager alloc]init];
