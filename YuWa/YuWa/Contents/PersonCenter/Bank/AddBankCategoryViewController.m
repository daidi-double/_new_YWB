...
- (void)userExplain{
    ...
    text2.text = @"没有预留手机号码忘记或者已停用，请联系银行客服更新处理";
    ...
    UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(cancelResponder)];
...
- (IBAction)seeRegreementAction:(UIButton *)sender {
    UserAgreenmentViewController * agreementVC = [[UserAgreenmentViewController alloc]init];
    agreementVC.staus = 0;   
...
- (void)requesData{
    ...
    NSDictionary * pragram = @{@"user_id":@([UserSession instance].uid),@"device_id":[JWTools getUUID],@"token":[UserSession instance].token,@"phone":_phoneNumberTF.text,@"user_type":@(1)};
