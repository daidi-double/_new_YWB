...

{
    ...
    if (self) {
        NSArray *array = [[NSBundle mainBundle] loadNibNamed:@"MovieAddComment1" owner:self options:nil];
...
-(void)changeStarForegroundViewWithPoint:(CGPoint)point{
    ...
    if(count==0){
        count = 1;
        [self.img_star1 setImage:[UIImage imageNamed:@"home_halfStar@2x.png"]];
    }
    self.count = count;
...
-(NSInteger)changeImg:(float)x image:(UIImageView*)img{
    if(x> img.frame.origin.x + img.frame.size.width/2){
        [img setImage:[UIImage imageNamed:@"home_lightStar@2x.png"]];
        return 2;
    }else if(x> img.frame.origin.x){
        [img setImage:[UIImage imageNamed:@"home_halfStar@2x.png"]];
        return 1;
//        [self setImageAnimation:img];
    }else{
        [img setImage:[UIImage imageNamed:@"home_grayStar@2x.png"]];
        return 0;
    }
}
